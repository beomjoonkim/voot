...
class PlaceGAIL(GAILPolicy):
  def __init__(self,\
               ...
               g_lr,\
               tau,architecture):
    super(PlaceGAIL,self).__init__(sess,\
                                   dim_a,\
                                   dim_misc,\
                                   dim_cvec,\
                                   weight_dir,\
  ...
  def createDisc(self):
    ...
    A_H  = Reshape( (self.n_key_confs,dense_num,1))(A_H)
  ...
  def createDisc1(self):
    ...
    X_H = Reshape( (self.n_key_confs,6+3,1))(XW)
    C_H = Reshape( (self.n_key_confs,self.dim_cvec[-1],1))(self.c_input)
    XC_H = Concatenate(axis=2)([X_H,C_H])
    H = Conv2D(filters=n_filters,\
               kernel_size=(1,self.dim_cvec[-1]+6+3),\
               strides=(1,1),
  ...
  def createV1(self):
    ...
    X_H = Reshape( (self.n_key_confs,3,1))(XW)
    C_H = Reshape( (self.n_key_confs,self.dim_cvec[-1],1))(self.c_input)
    XC_H = Concatenate(axis=2)([X_H,C_H])
    H = Conv2D(filters=n_filters,\
               kernel_size=(1,self.dim_cvec[-1]+3),\
               strides=(1,1),
               activation='relu')(XC_H)
    H = Conv2D(filters=n_filters,
                kernel_size=(1,1),
