...
class PickSOAPSeparate(Policy):
  def __init__(self,sess,
    ...
    super(PickSOAPSeparate,self).__init__(sess,\
                                   dim_a,\
                                   dim_misc,\
                                   dim_cvec,\
                                   weight_dir,\
  ...
  def createGen(self):
    ...
    # c0 and opose representation learning
    W_c0_opose = W[:,0:6]
    W_H_c0_opose  = Dense(dense_num,activation='relu')(W_c0_opose)
    W_H_c0_opose = Dense(3,activation='relu')(W_H_oshape) # this is supposed to represent base pose

    
    W_oshape   = W[:,6:9]
    W_H_oshape = Dense(dense_num,activation='relu')(W_oshape)
    W_H_oshape = Dense(3,activation='relu')(W_H_oshape) # this is supposed to represent grasp

    #Z_H = Dense(dense_num,activation='relu')(self.z)
    a_gen_output  = Concatenate()([W_H_c0_opose,W_H_oshape])
