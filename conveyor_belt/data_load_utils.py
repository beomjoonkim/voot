...
def format_RL_data( trajs,n_data=None ):
  ...
  if 'all_states' not in locals():
    ...
    all_states = np.array(all_states); all_states = all_states.reshape((n_data,n_konf,2))
    all_actions = np.array(all_actions); all_actions = all_actions.reshape((n_data,3))
    all_rewards = np.array(all_rewards); all_rewards = all_rewards.reshape((n_data,1))
    all_sprime = np.array(all_sprime);all_sprime = all_sprime.reshape((n_data,n_konf,2))
    all_sum_rewards = np.array(all_sum_rewards); all_sum_rewards = all_sum_rewards.reshape((n_data,1))
