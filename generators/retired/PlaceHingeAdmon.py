...
class PlaceHingeAdmon():
  def __init__(self,sess,dim_data,dim_context,save_folder,\
    ...
    self.fake_w_input = Input( shape = dim_context,name='w_fake',dtype='float32') # collision vector
  ...
  def createGAN(self):
    ...
    a_gen,a_gen_output,a_gen_output_fake = self.createGen()
    for l in disc.layers:
      l.trainable=False
    DG_output = disc([a_gen_output,a_gen_output_fake,self.w_input]) 
