...
class PlaceNoMisc(Policy):
  def __init__(self,\
               ...
               d_lr,\
               g_lr):
    super(PlaceNoMisc,self).__init__(sess,\
                                   dim_a,\
                                   dim_misc,\
                                   dim_cvec,\
                                   weight_dir,\
  ...
  def createDisc(self):
    ...
    dense_num = 64
    init_ = self.initializer
    X   = RepeatVector(self.n_key_confs)(self.a_input)
    X_H = Reshape( (self.n_key_confs,self.dim_a,1))(X)
    C_H = Reshape( (self.n_key_confs,self.dim_cvec[-1],1))(self.c_input)
    XC_H = Concatenate(axis=2)([X_H,C_H])
    H = Conv2D(filters=n_filters,\
               kernel_size=(1,self.dim_a+self.dim_cvec[-1]),\
               strides=(1,1),
