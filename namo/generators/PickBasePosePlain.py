...
import time
from generators.PickBaseXYPlain import separate_misc_data



class PickBasePosePlain(Policy):
  def __init__(self,sess,
    ...
    super(PickBasePosePlain,self).__init__(sess,\
                                   dim_a,\
                                   dim_misc,\
                                   dim_cvec,\
                                   weight_dir,\
  ...
  def createGen(self):
    ...
    """
    X = self.misc_input
    H = Dense(dense_num,activation='relu')(X)
  ...
  def createDisc(self):
    # This is a function of x and misc, 
    #   where x=[pick_base_pose,grasp] and misc = [c0,p_obj,o_shape]
    init_ = self.initializer
    dense_num = 64
  ...
  def separate_misc_data(self,misc_data):
    c0     = misc_data[:,0:3] # c0
    opose  = misc_data[:,3:6] 
  ...
  def predict(self,cvec,misc,n_samples=1):    
    ...
    if misc.shape[0] == 1 and n_samples > 1:
      ...
      rel_xyth  = self.a_scaler.inverse_transform(self.a_gen.predict([a_z,miscs,cvecs]))
    else:
      a_z     = noise(misc.shape[0],self.dim_a)
      rel_xyth  = self.a_scaler.inverse_transform(self.a_gen.predict([a_z,misc,cvec]))

    theta,height_portion,depth_portion = sample_grasp_parameters(n_smpls=n_samples)  
    rel_robot_xyth = rel_xyth
    if n_samples>1:
      action = np.hstack([theta[:,None],height_portion[:,None],\
                          depth_portion[:,None],rel_robot_xyth])
    else:
      action = np.hstack([theta,height_portion,depth_portion,rel_robot_xyth.squeeze()])[None,:]
    return action
